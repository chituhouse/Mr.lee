...
class Skill2Generator {
   ...
   * 阶段 B：深度生成（高+中适配）
   */
  async generateQuotes(highMidTopics, platformName) {
    if (!highMidTopics.length) {
      logger.info(`[Skill2b] ${platformName} 无高/中适配话题`);
      return [];
    }

    logger.info(`[Skill2b] ${platformName} 深度生成 ${highMidTopics.length} 条金句（高/中适配）`);

    const topicList = highMidTopics.map((t, i) =>
      `${i+1}. ${t.title} (适配度:${t.rating}) | 热度:${t.hot} 增长:${t.growth}`
    ).join("\n");

    const userMsg = `为${platformName}高/中适配选题生成理由和金句：\n\n${topicList}`;
  ...
  async process(topics, platformName, preferenceContext = "") {
    ...
    // Step 3: 深度生成（高+中）
    const highMid = [...high, ...mid];
    const enrichedHighMid = await this.generateQuotes(highMid, platformName);

    // Step 4: 合并结果（低不生成金句）
    return [
      ...enrichedHighMid,
      ...low.map(t => ({ ...t, reason: "", quote: "" })),
    ];
  }
}
